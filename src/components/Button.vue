...
const sizeClasses = computed(() => {
  if (!props.size) {
    return 'h-[32px] py-[8px] px-[15px] text-sm rounded-[8px]'
  } else {
    if (props.size === 'large') {
...
<template>
<button class="shadow-sm inline-flex items-center justify-center disabled:opacity-50 disabled:pointer-events-none"
        :class=[Colorclass,sizeClasses] :disabled=disabled||loading>
  <Icon v-if="iconWithLoading" :icon="iconWithLoading" class="mr-2"></Icon>
  <slot></slot>
</button>
</template>
